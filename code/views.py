...
class AppointmentCreateView(CreateView):
    """Allow patients to book a new appointment."""
    model = Appointment
...
class PastAppointmentsListView(ListView):
    """List past appointments for the logged-in patient."""
    model = Appointment
    template_name = 'appointments/past_appointments.html'
...
#                     .order_by('-created_at'))  # newest first&#8203;:contentReference[oaicite:0]{index=0}
#     return render(request, 'doctor_dashboard.html', {'appointments': appointments})


@login_required
def doctor_dashboard(request):
    # Get doctor profile or return 404
...
def update_appointment_status(request, appointment_id, new_status):
    ...
    appointment = get_object_or_404(
        Appointment,
        id=appointment_id,
        doctor__user=request.user  # Ensure doctor owns the appointment
    )
    ...
    if new_status in valid_transitions.get(appointment.status, []):
